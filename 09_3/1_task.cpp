...
//FIX_ME: цикл использовал i += 2 без учёта однобайтовых символов (пробел = 1 байт)
//— при наличии пробела смещение сбивалось и всё что после пробела не распознавалось;
//добавлена UTF-8-корректная итерация и конвертация в верхний регистр
//for (size_t i = 0; i < text.size(); i += 2) { ... }

// Переводит строчные кириллические буквы UTF-8 в заглавные.
// Кириллические символы в UTF-8 занимают ровно 2 байта (0xD0/0xD1 + продолжение).
// Строчные а-я: U+0430–U+044F; заглавные А-Я: U+0410–U+042F (разница U+0020).
...
int main() {
    ...
    //FIX_ME: строчные буквы не давали вывода — карта содержит только заглавные
    //буквы, а пользовательский ввод не приводился к верхнему регистру
    //getline(cin, text);  (без конвертации)
    text = toUpperCyrillic(text);

    for (size_t i = 0; i < text.size(); ) {
        unsigned char c = static_cast<unsigned char>(text[i]);
        if (c == ' ') {
            //FIX_ME: пробел между словами не обрабатывался — по условию задачи
            //должен выводиться символ "/"
            cout << "/ ";
            ++i;
